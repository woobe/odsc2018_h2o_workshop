---
title: "Classification Example: Diabetes"
author: "Jo-fai (Joe) Chow - joe@h2o.ai"
date: "H2O + LIME Workshop at ODSC 2018"
output: 
  html_document: 
    df_print: kable
    fig_height: 10
...
# Data Prep - Pima Indians Diabetes
...
```{r}
# H2O dataframe
# Note: use as.h2o(...) for small data frame only
# For real use cases, use h2o.importFile(...) 
h_diabetes = as.h2o(PimaIndiansDiabetes)
```

```{r}
# Check first six samples
head(PimaIndiansDiabetes)
```
...
model_automl = h2o.automl(x = features,
                          ...
                          max_runtime_secs = 300,  # Max time
                          # max_models = 100,      # or Max no. of models
                          stopping_metric = "logloss", # Metric to optimize
                          stopping_rounds = 5,     # control early stopping
                          project_name = "automl_class", # useful if you want to add more models in future
...
```{r}
# Default GBM Model
h2o.logloss(h2o.performance(model_gbm, newdata = h_test))
```

```{r}
# Best model from AutoML
h2o.logloss(h2o.performance(model_automl@leader, newdata = h_test)) # lower logloss = better
```
...
explanations = lime::explain(x = d_samp,
                             ...
                             n_features = 8) # Look top n features
```
...
Replace `PimaIndiansDiabetes` with your own data. Good luck!
